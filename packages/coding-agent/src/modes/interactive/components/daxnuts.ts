 ...
 * POWERED BY DAXNUTS：OpenCode + Kimi K2.5 的彩蛋组件。
 *
 * A heartfelt tribute to dax (@thdxr) for providing free Kimi K2.5 access via OpenCode.
 * 向通过 OpenCode 提供免费 Kimi K2.5 访问的 dax (@thdxr) 致意。
 */
...
// dax 的 32x32 RGB 图像，每个像素以 3 字节十六进制编码。
const DAX_HEX =
...
function buildImage(): string[] {
	...
	// 使用半块字符 ▄：背景色表示上方像素，前景色表示下方像素，从而把两行像素压缩为一行终端字符。
...
export class DaxnutsComponent implements Component {
	...
	invalidate(): void {
		// 外部主题或布局失效时清除宽度缓存，下一次 render 会重新着色和居中。
		this.cachedWidth = 0;
	}

	private startAnimation(): void {
		// 每帧只推进 tick 并请求重绘；达到上限后停止 interval，最终画面保持静态。
	...
	render(width: number): string[] {
		// 宽度和 tick 均未变化时复用完整 ANSI 行，避免高频 TUI 重绘重复构造图像。
		...
		const center = (s: string) => {
			// 居中计算先移除 ANSI SGR 序列，颜色控制码不计入可见宽度。
		...
		// 图像揭示到后半段后再显示文字，空行确保动画期间整体布局高度不跳动。
	...
	dispose(): void {
		// 组件移除时终止 interval，避免后台计时器继续触发无效重绘并阻止进程退出。
		this.stopAnimation();
	}
}
