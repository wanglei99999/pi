...
> **修订说明（2026-07-20）**：fork 并入上游 v0.80.10（`3da591ab`）后，本篇写作时预告的迁移落地了。§2.2、§2.3、§5、§10 已按新代码**就地改写**，这四处的行号基于 fork 当前 HEAD（`3da591ab` + 注释提交，个别行号有几行浮动）；其余章节（SSE 解析、消息转换、预算学、faux 等）不受重构影响，行号仍基于 `3f9aa5d1`。
...
### 2.2 compat.ts：旧世界只剩边角（v0.80.10 修订）

`compat.ts`（283 行）的头注释仍自称 "Temporary compatibility entrypoint"，但本篇初版描述的"pi CLI 每次调用都进 compat"已成历史。v0.80.10 起，**CLI 主链路不再穿过 compat**：`sdk.ts:3` 从 compat 只导入类型和 `clampThinkingLevel`，请求走 coding-agent 的 `ModelRuntime`（03 篇第 10 章）直达 Models 集合。

compat 还活着的三个理由：

- **裸 Agent 的默认路径**：`packages/agent/src/agent-loop.ts:12` 的默认 streamFn 仍来自 compat——不注入 streamFn 直接 `new Agent(...)` 的外部用户走这条；pi CLI 总是注入自己的闭包，不受影响。
- **扩展 API**：`registerApiProvider`（compat.ts:137）让扩展注册自定义 API 实现，faux 测试体系（第 12 章）也依赖它。
- **静态目录读取**：`getModel`/`getModels` 等无鉴权的目录查询（compat.ts:233 的模块级 `builtinModels()` 实例）。

内部分叉也简化了：初版引用的 `shouldUseBuiltinModels` 已被 `getBuiltinProviderForModel`（compat.ts:255-267）取代——内置模型走 Models 集合，覆盖注册的 api 走旧注册表并用 `withEnvApiKey`（compat.ts:240）从环境变量补密钥。**判断**：compat 如今名副其实是历史适配器，读代码时可以放心把 Models 集合当唯一主线；旧的全局 OAuth 注册表（`registerOAuthProvider` 一族）已随迁移整体删除。

### 2.3 密钥解析回到了 pi-ai（v0.80.10 修订）

本篇初版在这里写过"pi CLI 的鉴权不走本包的 auth/ 体系"——这个论断被 v0.80.10 精确反转，值得当作案例记录。旧链路是 coding-agent 的 `modelRegistry.getApiKeyAndHeaders` 解析好密钥再显式塞进 options；新链路里 sdk.ts 的 streamFn（sdk.ts:321-351）只做两件事：从 settings 补超时/重试参数，注入一个 `transformHeaders` 回调（归因 headers + 扩展钩子），然后调 `modelRuntime.streamSimple`。**认证解析发生在 pi-ai 内部**：`ModelsImpl.applyAuth`（models.ts:515）→ `resolveProviderAuth`（auth/resolve.ts:42），OAuth 锁内刷新、env 合并全在这条线上。coding-agent 的 AuthStorage 没有消失，但降级成了注入给 Models 的 `CredentialStore` 实现（第 10 章）。

当年 `auth/resolve.ts` 注释里"抄的是 today's AuthStorage 模式"的提炼，如今真的回头替换了原型——初版说的"提炼完成但尚未回头替换"补完了后半句。附带的收益：初版吐槽过 sdk.ts 鉴权失败直接 `throw` 违反 StreamFn 契约的灰色地带也消失了——现在未配置 provider 时 `applyAuth` 抛 `ModelsError`，但它发生在 `lazyStream` 的异步装配闭包内（第 4 章的机制），自动兑现为 error 事件流，契约由结构保证而不是靠上层安全网。
...
`createProvider`（models.ts:617-684，v0.80.10 修订行号）做四件事：`api` 参数接受单实现或"按 model.api 分发的 map"（混合 API 厂商如 GitHub Copilot 用后者，models.ts:635 的 `apiFor`）；找不到实现时**不 throw**，`dispatch`（637-648）用 lazyStream 返回一条 error 流（契约的又一次兑现）；动态厂商的 `fetchModels` 被包上**持久化**——先从注入的 `ModelsStore` 恢复上次目录（离线也有模型列表），网络拉取成功才覆盖并落盘（657-678）；并发去重仍是 `inflightRefresh ??=`（659），同时多次刷新共享一个 in-flight promise。

对比初版：`refreshModels` 从"纯获取"升级成"恢复 + 获取 + 持久化"三段式，新签名收一个 `RefreshModelsContext`（凭据、限定本 provider 的 store 视图、allowNetwork/force/signal）——动态目录从会话级内存缓存变成了跨启动的本地缓存。

### 5.2 Models 集合：鉴权注入点（v0.80.10 修订）

`ModelsImpl`（models.ts:261）持有 providers 表和三个可注入依赖：`CredentialStore`（凭据）、`ModelsStore`（动态目录）、`AuthContext`（env/文件探测）。默认全是内存实现——持久化由宿主注入，pi CLI 注入的就是 coding-agent 的 AuthStorage 和 models-store.json（03 篇第 10 章）。

`streamSimple`（models.ts:565）= lazyStream 包住"查厂商 → applyAuth → 转发"。applyAuth（515-540）的合并规则值得记：**显式 options 按字段赢，headers/env 按 key 合并，`transformHeaders` 回调最后执行**——
...
初版这里写的是"pi CLI 已放显式 apiKey，本包凭据存储实际不参与"——现在恰恰相反：CLI 不再预解析密钥，`resolveProviderAuth`（auth/resolve.ts:42）在这条路上真实执行，overrides 短路只服务 `--api-key` 场景。`transformHeaders` 是新加的钩子位，coding-agent 用它把归因 headers 和扩展的 `before_provider_headers` 注入到已合并的最终 header 上（sdk.ts:339-349）。

集合层的其他成员也值得扫一眼：`refresh`（320）并发刷新全部动态厂商，失败进 errors 映射不 reject，还会降级 `allowNetwork: false` 再跑一次尽力从缓存恢复；`getAvailable`（444）= 逐厂商无副作用认证检查 + 可选的 `filterModels` 凭据过滤；`login`/`logout`（482/499）把登录结果经 `credentials.modify` 这一唯一写入口持久化。

### 5.3 calculateCost：一处刻意的可变操作

```typescript
// models.ts:701-721（v0.80.10 修订行号）
...
注意它**就地改写** `usage.cost`——SSE 主循环里每个 message_delta 都要重算成本（第 9 章），热路径上避免了每次分配新对象。另外 1h 缓存写按 2 倍基础输入价计费的领域知识就藏在这四行里。v0.80.10 新增了**阶梯价**：`model.cost.tiers` 按累计输入 token 档位切换费率（702-710 选档），GitHub Copilot 的长上下文分档定价靠它表达。

### 5.4 thinking level 的协商

`getSupportedThinkingLevels`（models.ts:726）+ `clampThinkingLevel`（738+）处理"用户要 high 但模型只有 medium"的降级：thinkingLevelMap 里 `null` 表示该档位不可用，`xhigh` 和 v0.80.10 新增的 `max` 默认不可用除非显式映射（models.ts:733）。看 claude-fable-5 的目录条目（providers/anthropic.models.ts:15）：`thinkingLevelMap: {"off":null,"xhigh":"xhigh"}`——off 是 null 意味着**这个模型无法关闭思考**，xhigh 显式开放。clamp 算法先向上找再向下找（models.ts:747-757），保证总能落到一个合法档位。
...
> v0.80.10 起这套抽象不再是"备用的新世界"——pi CLI 的每次请求都真实走它（§2.3）。本节行号已按新代码修订。

鉴权体系四个接口（auth/types.ts）：`ProviderAuth = { apiKey?, oauth? }`（250-253，至少有一个——连无密钥的本地服务器也要提供 `resolve()` 报告"已配置"）；`ApiKeyAuth.resolve` 按"存储凭据 → env 变量"的顺序合并（auth/helpers.ts 的 `envApiKeyAuth`，11-27），另有无副作用的 `check`（types.ts:202）供列表展示时探测可用性而不执行命令；`OAuthAuth` 拆成 `refresh`（网络调用，会 throw）和 `toAuth`（纯派生）两步——**拆分的目的是让 Models 拥有加锁刷新的控制权**；`CredentialStore.modify` 是唯一写路径（70-105），每次写都是序列化的 read-modify-write。

刷新的并发安全（auth/resolve.ts:95-133）是教科书式的 double-checked locking：
...
有效令牌零加锁；过期令牌锁内复查再刷，跨进程（文件锁实现的 store）也不会双刷——refresh token 是单次使用的，双刷会把另一个进程的凭据作废。优先级规则（resolveProviderAuth，42-75）：显式 overrides.apiKey > 存储凭据 > 环境变量，且**存储凭据一旦存在就拥有该厂商**——刷新失败不会静默回落到 env（33-40 行注释），避免"用户以为在用订阅、实际在烧 API key"。

### 10.2 OAuth 流程实现（auth/oauth/anthropic.ts）

> v0.80.10 把整个 `utils/oauth/` 目录迁到了 `auth/oauth/`，本节路径已更新；流程逻辑本身没变。目录里还多了两个新流程：`xai.ts`（纯设备码，无回调服务器）和 `radius.ts`（网关 OAuth，端点从 `/v1/oauth` 动态发现，浏览器/设备码双通道）——两个文件都有中文注释可对照读。

标准 PKCE 授权码流，几个工程细节：callback 服务器固定端口 53692（36 行）；**state 直接用 PKCE verifier**（255 行）省一个随机数；`CLIENT_ID` 用 base64 存储（32，防 grep 而非防逆向）；令牌过期时间提前 5 分钟记账（232 行 `- 5 * 60 * 1000`，消抖网络延迟）；**本地 callback 与手动粘贴赛跑**（240-308）——浏览器在另一台机器时用户可以把重定向 URL 粘回终端，`manual_code` prompt 与 callback 服务器互相 cancel（`manualAbort`，240/269/308）。scope 里 `user:inference` 是推理授权，`user:sessions:claude_code`（40 行）对应第 7 章的伪装身份。

### 10.3 env-api-keys.ts：旧世界的 env 映射表

如今只剩 compat 路径引用的平面函数（v0.80.10 后新主线的 env 解析走各 provider 的 `ApiKeyAuth.resolve`）：`envMap` 一厂商一变量；三个特例——anthropic 的 `ANTHROPIC_OAUTH_TOKEN` 优先于 `ANTHROPIC_API_KEY`；google-vertex 和 amazon-bedrock 没有"key"概念，检测到 ADC 文件/AWS 凭据链时返回哨兵字符串 `"<authenticated>"`——上层只判真值，SDK 自己走凭据链。
...
1. **流函数同步返回，错误永远编码进流**：lazyStream 的 catch（lazy.ts:49-53）、dispatch 的无实现分支（models.ts:642-646）、API 实现的顶层 catch（anthropic-messages.ts:727-737）三层兜底，任何 setup/网络/解析失败都变成 `stopReason: "error"` 的消息事件。v0.80.10 后连"鉴权未配置"也走这条线（§2.3）。
...
- **改鉴权逻辑就是改 CLI 行为**（v0.80.10 反转，§2.3）：pi CLI 的密钥解析如今就在 pi-ai 的 auth/ 和 Models 集合里，动 `resolveProviderAuth`/`ApiKeyAuth` 会直接影响 CLI；compat.ts 只剩边角职责，别在它上面堆新功能。
...
本篇从 sdk.ts 的 streamFn 闭包向下走完了 provider 一侧。下一篇 `03-coding-agent-core.md` 从同一处向上走：这个闭包所在的 `createAgentSession` 如何装配 AgentSession、工具的 Operations 注入、JSONL 会话树的构建/fork、ModelRuntime 服务群（本篇 Models 集合在 coding-agent 侧的宿主，03 篇第 10 章）、settings 与 trust。初版第 2 章留下的问题——"ModelManager 迁移后 coding-agent 如何改用 Models 集合"——答案就是 03 篇第 10 章的 `ModelRuntime`。

---

*初版基于 commit 3f9aa5d1；§2.2/2.3、§5、§10 于 2026-07-20 按 v0.80.10（`3da591ab`）修订。ai 包的外层类型（Model/Context/AssistantMessageEvent/StreamFunction 契约）是跨包承诺，稳定；api 实现随厂商 API 演进频繁改动，行号漂移预期较快，但"装配层三层兜底 + 方言进 compat 开关 + Provider/Models 双层"的结构短期不会变。*
