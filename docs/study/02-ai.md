...
> **⚠️ v0.80.10 勘误（2026-07-20，fork 已并入上游 `3da591ab`）**：本篇写作时预告的迁移已经落地，以下论断已过时——
>
> 1. **§2.3 "密钥解析发生在 pi-ai 之外" 不再成立**。pi CLI 的请求路径现在是 `sdk.ts` streamFn → coding-agent 的 `ModelRuntime.streamSimple`（model-runtime.ts）→ pi-ai 的 `ModelsImpl`，认证解析（含 OAuth 锁内刷新）就发生在 pi-ai 的 `auth/resolve.ts`。旧的 `getApiKeyAndHeaders` 显式注入链已删除。
> 2. **§2.2 的 compat 分叉弱化**：`compat.ts` 仍在，但只剩类型/工具导入和"未注入 streamFn 的裸 Agent"默认路径；CLI 主链路不再穿过全局 api 注册表。全局 OAuth 注册表（`registerOAuthProvider` 等）已整体删除。
> 3. **§5 的 models.ts 已重写**：`Provider`/`Models`/`ModelsImpl` 新接口 + 可注入的 `CredentialStore`/`ModelsStore`；`utils/oauth/` 迁至 `auth/oauth/`（新增 xai.ts、radius.ts）。§10 描述的 auth/ 结构方向正确，但 API 细节以新代码为准。
>
> 新架构的走读见源码注释（model-runtime.ts、provider-composer.ts、models.ts 均有中文注释）；本篇其余章节（SSE 解析、消息转换、预算学、faux）不受影响。
